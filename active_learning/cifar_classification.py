...
def train_model(model, criterion, optimizer, dataloader, device, epochs=1):
    model.train()
...
def get_uncertainty(model, dataloader, device):
    model.eval()
    uncertainties = []
    with torch.no_grad():
        for data in dataloader:
...
def main():
    ...
    initial_loader = DataLoader(Subset(train_set, initial_idx), batch_size=64, shuffle=True)

    # Train the initial model
    print("Training without active learning")
    train_model(net, criterion, optimizer, initial_loader, device, epochs=30)

    # Evaluate the initial model
    initial_accuracy = evaluate_model(net, test_loader, device)
    print(f'Initial accuracy: {initial_accuracy:.2f}')

    
    # Active learning loop
    n_queries = 100 # the iterations to run the data cleaning
    query_size = 100 # the size of uncertainty samples during training

    print("Training with active learning")
    for i in range(n_queries):
        # Get the model's uncertainty on the training data
        train_loader = DataLoader(train_set, batch_size=64, shuffle=False)
        uncertainties = get_uncertainty(net, train_loader, device)
        
        # Select the most uncertain samples
        query_idx = np.argsort(-uncertainties)[:query_size]
        query_loader = DataLoader(Subset(train_set, query_idx), batch_size=64, shuffle=True)
        
        # Teach the model with the queried samples
        train_model(net, criterion, optimizer, query_loader, device, epochs=30)
...
if __name__ == '__main__':
    ...
    # The result of this sample show the benefit of active learning
    # The accuracy without active learning 0.1
    # The accuracy with active learning 
